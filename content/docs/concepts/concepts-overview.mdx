...
A [theme](/docs/concepts/theme) is a Next.js app which has [data files](/docs/concepts/data-files). It is used for creating Next.js websites that have a Sanity content management system. 

A theme can have any number of [modules](/docs/concepts/module). A module represents a business use case, and contains all the pages and sections to fulfill the use case. It has [module pages](/docs/concepts/module-page), each of which assigns sections to a page. Consequently, a [page](/concept/page) is "dumb", not directly determining its sections.
...
You may wish to have small reusable content types such as that can be used for UI components. These are called [elements](/docs/concepts/elements).


A theme can also have any number of [plugins](/docs/concepts/plugin). A plugin is a single module which is external to the theme. You may also need to store global data for your theme. The [global](/concept/global) content type can used for this purpose. Currently, Built.js themes and sites are limited to one common [layout](/concpets/layout) for all pages.
