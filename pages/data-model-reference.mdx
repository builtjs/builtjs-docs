...
| `sections`    | Array of objects | References Sections used in the ModulePage. Object has ```name``` and ```position``` properties. ```position``` property is a value representing the section's position in the page. |
...
App data describes the theme or plugin.
